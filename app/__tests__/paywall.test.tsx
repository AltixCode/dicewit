...
describe("Paywall", () => {
  ...
  it("opens the first benefit by default and lists what the purchase unlocks", async () => {
    const { getByText, queryByText } = await renderWithProviders(<Paywall />);
    expect(getByText(t("feat1Title"))).toBeTruthy();
    expect(getByText(t("feat1Desc"))).toBeTruthy();
    // The rest start collapsed — only the first row's description is visible.
    expect(queryByText(t("feat2Desc"))).toBeNull();
  });

  it("is independent — opening a second row leaves the first one open", async () => {
    const { getByText, getByLabelText } = await renderWithProviders(
      <Paywall />,
    );
    expect(getByText(t("feat1Desc"))).toBeTruthy();
    await fireEvent.press(getByLabelText(t("feat2Title")));
    expect(getByText(t("feat2Desc"))).toBeTruthy();
    expect(getByText(t("feat1Desc"))).toBeTruthy();
  });

  it("collapses a row that is tapped again", async () => {
    const { getByLabelText, queryByText } = await renderWithProviders(
      <Paywall />,
    );
    await fireEvent.press(getByLabelText(t("feat1Title")));
    expect(queryByText(t("feat1Desc"))).toBeNull();
  });
