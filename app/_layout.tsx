...
function RootNavigator() {
  ...
  const hydrateDice = useDiceStore((s) => s.hydrate);
  const hydrateGame = useGameStore((s) => s.hydrate);

  useEffect(() => {
    void initialize();
    // Both stores read from AsyncStorage, so a launch shows saved presets, the roll log and a
    // game in progress rather than an empty app that fills in a frame later.
    void hydrateDice();
    void hydrateGame();
    void SplashScreen.hideAsync();
  }, [initialize, hydrateDice, hydrateGame]);
  ...
  return (
    <>
      ...
      >
        ...
        <Stack.Screen name="scorecard" options={{ title: t('scorecardTitle') }} />
        <Stack.Screen
          name="paywall"
